...
export function ConversationRightSidebar({
  ...
  return (
    <aside ref={sidebarScrollRef} className="hidden xl:flex w-[320px] flex-shrink-0 flex-col min-h-0 border-l border-slate-200 dark:border-border-dark bg-slate-50 dark:bg-background-dark overflow-y-auto overflow-x-hidden">
      <div className="p-6 flex flex-col shrink-0">
        <div className="flex flex-col items-center text-center mb-4">
            ...
            >
              ...
              {selected?.isGroup && !selected?.avatar ? (
                <div className="w-full h-full rounded-full bg-[#4a4a4a] flex items-center justify-center border border-white/10 relative">
                  <span className="material-symbols-outlined text-5xl text-slate-500 dark:text-gray-400" aria-hidden>group</span>
                  {canEditGroupAvatar && (
                    <span className="absolute bottom-0 right-0 w-8 h-8 rounded-full bg-[#3a3a3a] border-2 border-background-dark flex items-center justify-center" aria-hidden>
                      <span className="material-symbols-outlined text-slate-900 dark:text-white text-lg">photo_camera</span>
                    </span>
                  )}
                </div>
              ) : (
          ...
          {selected?.isGroup && canEditGroupName && isEditingGroupName ? (
            <input
              ...
              className="w-full mt-3 text-lg font-bold text-slate-900 dark:text-white bg-white dark:bg-card-dark border border-slate-200 dark:border-border-dark rounded-xl px-3 py-2 text-center focus:ring-2 focus:ring-primary outline-none"
            ...
            >
              <h4 className="text-lg font-bold text-slate-900 dark:text-white truncate">{selected?.name}</h4>
              <span className="material-symbols-outlined text-slate-500 dark:text-gray-400 text-base shrink-0 opacity-0 group-hover:opacity-100 transition-opacity" aria-hidden>edit</span>
            </button>
          ) : (
            <h4 className="text-lg font-bold text-slate-900 dark:text-white mt-3">{selected?.name}</h4>
          )}
          {selected?.isGroup && (
        ...
        {rightBarSearchQuery.trim() && (
          <div className="mb-4">
            <h5 className="text-xs font-bold text-slate-400 dark:text-gray-500 uppercase tracking-widest mb-2">{t('messages.searchResults')} ({rightBarSearchResults.length})</h5>
              ...
              {rightBarSearchResults.length === 0 ? (
                ...
                rightBarSearchResults.map((r) => (
                  <button
                    ...
                    className="w-full text-left p-2.5 rounded-lg hover:bg-slate-100 dark:hover:bg-card-dark border border-transparent hover:border-border-dark transition-colors group"
                  >
      ...
      <div className="px-6 space-y-6 pb-6 shrink-0">
        {selected?.isGroup && (
          <div>
            <button type="button" onClick={() => toggleSection('members')} className="w-full flex items-center justify-between text-left mb-2 group">
              <h5 className="text-xs font-bold text-slate-400 dark:text-gray-500 uppercase tracking-widest group-hover:text-gray-400">{t('messages.membersSection')}</h5>
              ...
              {Array.isArray(selected.members) && selected.members.length > 0 ? selected.members.map((m) => {
                ...
                return (
                  <div key={m.userId} className={`flex items-center gap-2 p-3 rounded-xl hover:bg-card-dark transition-colors group relative ${openMemberMenuUserId === m.userId ? 'z-20' : ''}`}>
                    <Link to={ROUTES.PROFILE_USER(m.userId)} className="flex items-center gap-3 min-w-0 flex-1 text-slate-900 dark:text-white">
                      <div className="w-9 h-9 rounded-full overflow-hidden shrink-0 bg-white dark:bg-card-dark">
                        {m.avatar ? (
                    ...
                    {showMenu && (
                      <div className="relative shrink-0" ref={openMemberMenuUserId === m.userId ? memberMenuRef : null}>
                        <button
                          ...
                          className="p-1.5 rounded-lg text-slate-500 dark:text-gray-400 hover:text-slate-900 dark:hover:text-white hover:bg-white/10 transition-colors"
                ...
                <p className="text-sm text-slate-400 dark:text-gray-500 py-2 text-center">—</p>
              )}
            </div>
            </>
            )}
        ...
        {selected?.isGroup && (selected?.myRole === 'host' || (selected?.myRole === 'admin' && selected?.groupPermissions?.adminCanAssignUserPermissions) || (selected?.myRole === 'user' && selected?.groupPermissions?.userCanEditGroupInfo)) && (
          <div>
            <button type="button" onClick={() => toggleSection('settings')} className="w-full flex items-center justify-between text-left mb-2 group">
              <h5 className="text-xs font-bold text-slate-400 dark:text-gray-500 uppercase tracking-widest group-hover:text-gray-400">{t('messages.settingsSection')}</h5>
            ...
            <div className="space-y-1">
              <button type="button" onClick={() => onOpenGroupSettings?.()} className="w-full flex items-center justify-between p-3 rounded-xl hover:bg-slate-100 dark:hover:bg-card-dark transition-colors text-slate-900 dark:text-white">
                <span className="text-sm font-medium">{t('messages.groupSettings')}</span>
                <span className="material-symbols-outlined text-slate-500 dark:text-gray-400">settings</span>
              </button>
            </div>
            )}
          </div>
        ...
        <div>
          <button type="button" onClick={() => toggleSection('options')} className="w-full flex items-center justify-between text-left mb-2 group">
            <h5 className="text-xs font-bold text-slate-400 dark:text-gray-500 uppercase tracking-widest group-hover:text-gray-400">{t('messages.options')}</h5>
          ...
          <div className="space-y-1">
            <div className="relative">
              <button type="button" onClick={() => setOpenSettingsMenu((v) => (v === 'mute' ? null : 'mute'))} className="w-full flex items-center justify-between p-3 rounded-xl hover:bg-slate-100 dark:hover:bg-card-dark transition-colors text-slate-900 dark:text-white">
              ...
              {openSettingsMenu === 'mute' && (
                <div className="absolute top-full left-0 right-0 mt-0.5 py-1 rounded-xl bg-white dark:bg-card-dark border border-slate-200 dark:border-border-dark shadow-xl z-20">
                  {selected?.muted ? (
                    <button type="button" onClick={() => applyConversationSettings({ mutedUntil: null })} className="w-full px-4 py-2 text-left text-sm text-slate-900 dark:text-white hover:bg-white/10 flex items-center gap-2">
                      <span className="material-symbols-outlined text-lg">notifications</span>
                      {t('messages.turnNotificationsOn')}
                    </button>
                  ) : (
                    <>
                      <button type="button" onClick={() => applyConversationSettings({ mutedUntil: getSettingsUntil('1h') })} className="w-full px-4 py-2 text-left text-sm text-slate-900 dark:text-white hover:bg-white/10">{t('messages.muteFor1h')}</button>
                      <button type="button" onClick={() => applyConversationSettings({ mutedUntil: getSettingsUntil('8h') })} className="w-full px-4 py-2 text-left text-sm text-slate-900 dark:text-white hover:bg-white/10">{t('messages.muteFor8h')}</button>
                      <button type="button" onClick={() => applyConversationSettings({ mutedUntil: getSettingsUntil('forever') })} className="w-full px-4 py-2 text-left text-sm text-slate-900 dark:text-white hover:bg-white/10">{t('messages.muteUntilTurnOn')}</button>
                    </>
                  )}
                </div>
              )}
            ...
            <div className="relative">
              <button type="button" onClick={() => setOpenSettingsMenu((v) => (v === 'disappearing' ? null : 'disappearing'))} className="w-full flex items-center justify-between p-3 rounded-xl hover:bg-slate-100 dark:hover:bg-card-dark transition-colors text-slate-900 dark:text-white">
              ...
              {openSettingsMenu === 'disappearing' && (
                <div className="absolute top-full left-0 right-0 mt-0.5 py-1 rounded-xl bg-white dark:bg-card-dark border border-slate-200 dark:border-border-dark shadow-xl z-20">
                  {selected?.disappearing ? (
                    <button type="button" onClick={() => applyConversationSettings({ disappearingUntil: null })} className="w-full px-4 py-2 text-left text-sm text-slate-900 dark:text-white hover:bg-white/10 flex items-center gap-2">
                      <span className="material-symbols-outlined text-lg">timer_off</span>
                      {t('messages.disappearingOff')}
                    </button>
                  ) : (
                    <>
                      <button type="button" onClick={() => applyConversationSettings({ disappearingUntil: getSettingsUntil('1h'), disappearingDurationSeconds: getDisappearingDurationSeconds('1h') })} className="w-full px-4 py-2 text-left text-sm text-slate-900 dark:text-white hover:bg-white/10">{t('messages.muteFor1h')}</button>
                      <button type="button" onClick={() => applyConversationSettings({ disappearingUntil: getSettingsUntil('8h'), disappearingDurationSeconds: getDisappearingDurationSeconds('8h') })} className="w-full px-4 py-2 text-left text-sm text-slate-900 dark:text-white hover:bg-white/10">{t('messages.muteFor8h')}</button>
                      <button type="button" onClick={() => applyConversationSettings({ disappearingUntil: getSettingsUntil('24h'), disappearingDurationSeconds: getDisappearingDurationSeconds('24h') })} className="w-full px-4 py-2 text-left text-sm text-slate-900 dark:text-white hover:bg-white/10">{t('messages.muteFor24h')}</button>
                      <button type="button" onClick={() => applyConversationSettings({ disappearingUntil: getSettingsUntil('forever'), disappearingDurationSeconds: getDisappearingDurationSeconds('forever') })} className="w-full px-4 py-2 text-left text-sm text-slate-900 dark:text-white hover:bg-white/10">{t('messages.disappearingUntilTurnOff')}</button>
                    </>
                  )}
                </div>
              )}
          ...
          <div className="space-y-1 mt-4">
            <button type="button" onClick={() => setShowDeleteAllConfirm(true)} className="w-full flex items-center justify-between p-3 rounded-xl hover:bg-slate-100 dark:hover:bg-card-dark transition-colors text-slate-900 dark:text-white">
              <span className="text-sm font-medium">{t('messages.deleteAllMessages')}</span>
              <span className="material-symbols-outlined text-slate-500 dark:text-gray-400">delete_sweep</span>
            </button>
            {selected?.isGroup && selected?.myRole === 'host' && onDisbandGroup && (
              <button type="button" onClick={onDisbandGroup} className="w-full flex items-center justify-between p-3 rounded-xl hover:bg-slate-100 dark:hover:bg-card-dark transition-colors text-red-400 hover:text-red-300">
                <span className="text-sm font-medium">{t('messages.disbandGroup')}</span>
                <span className="material-symbols-outlined text-slate-500 dark:text-gray-400">group_off</span>
              </button>
            )}
            {selected?.isGroup && onLeaveGroup && (
              <button type="button" onClick={onLeaveGroup} className="w-full flex items-center justify-between p-3 rounded-xl hover:bg-slate-100 dark:hover:bg-card-dark transition-colors text-amber-400 hover:text-amber-300">
                <span className="text-sm font-medium">{t('messages.leaveGroup')}</span>
                <span className="material-symbols-outlined text-slate-500 dark:text-gray-400">logout</span>
              </button>
            )}
          </div>
          </>
        ...
        {(onReport || (!selected?.isGroup && onBlock)) && (
          <div>
            <button type="button" onClick={() => toggleSection('privacy')} className="w-full flex items-center justify-between text-left mb-2 group">
              <h5 className="text-xs font-bold text-slate-400 dark:text-gray-500 uppercase tracking-widest group-hover:text-gray-400">{t('messages.privacySupport')}</h5>
            ...
            <div className="space-y-1">
              {!selected?.isGroup && onBlock && (
                <button type="button" onClick={onBlock} className="w-full flex items-center justify-between p-3 rounded-xl hover:bg-slate-100 dark:hover:bg-card-dark transition-colors text-slate-900 dark:text-white">
                  <span className="text-sm font-medium">{t('messages.block')}</span>
                  <span className="material-symbols-outlined text-slate-500 dark:text-gray-400">block</span>
                </button>
              )}
              {onReport && (
                <button type="button" onClick={onReport} className="w-full flex items-center justify-between p-3 rounded-xl hover:bg-slate-100 dark:hover:bg-card-dark transition-colors text-slate-900 dark:text-white">
                  <span className="text-sm font-medium">{t('messages.report')}</span>
                  <span className="material-symbols-outlined text-slate-500 dark:text-gray-400">flag</span>
                </button>
              )}
            </div>
            )}
        ...
        <div>
          <h5 className="text-xs font-bold text-slate-400 dark:text-gray-500 uppercase tracking-widest mb-4">{t('messages.mediaSection')}</h5>
          <div className="grid grid-cols-3 gap-2">
            {rightBarMedia.length === 0 ? (
              <p className="col-span-3 text-slate-400 dark:text-gray-500 text-sm py-2">—</p>
            ) : (
              rightBarMedia.slice(0, rightBarMediaVisible).map((a, i) =>
                a.type?.startsWith('video/') ? (
                  <a key={`${a.url}-${i}`} href={a.url} target="_blank" rel="noopener noreferrer" className="aspect-square bg-white dark:bg-card-dark rounded-lg overflow-hidden border border-slate-200 dark:border-border-dark flex items-center justify-center hover:opacity-90">
                    <span className="material-symbols-outlined text-primary">play_circle</span>
                  </a>
                ) : (
                  <div key={`${a.url}-${i}`} className="relative aspect-square bg-white dark:bg-card-dark rounded-lg overflow-hidden border border-slate-200 dark:border-border-dark group/thumb">
                    ...
                    <div className="absolute top-1.5 right-1.5 flex items-center gap-1 opacity-0 group-hover/thumb:opacity-100 transition-opacity">
                      <span className="w-7 h-7 rounded-full bg-black/50 flex items-center justify-center cursor-pointer shrink-0" title={t('messages.viewOriginalMessage')} onClick={(e) => { e.stopPropagation(); scrollToMessage(a.messageId) }}>
                        <span className="material-symbols-outlined text-slate-900 dark:text-white text-sm">visibility</span>
                      </span>
                      <span className="w-7 h-7 rounded-full bg-black/50 flex items-center justify-center cursor-pointer shrink-0" title={t('common.download')} onClick={(e) => { e.stopPropagation(); downloadAttachment(a.url, a.name || 'image') }}>
                        <span className="material-symbols-outlined text-slate-900 dark:text-white text-sm">download</span>
                      </span>
                    </div>
                  </div>
                )
            ...
            {rightBarMedia.length > rightBarMediaVisible && (
              <button type="button" disabled={loadMoreMedia} onClick={setRightBarMediaVisibleCount} className="col-span-3 py-2 rounded-lg bg-white dark:bg-card-dark hover:bg-white/10 text-sm text-primary font-medium disabled:opacity-50 flex items-center justify-center gap-1">
        ...
        <div>
          <h5 className="text-xs font-bold text-slate-400 dark:text-gray-500 uppercase tracking-widest mb-4">{t('messages.fileSection')}</h5>
          <div className="space-y-1">
            {rightBarFiles.length === 0 ? (
              <p className="text-slate-400 dark:text-gray-500 text-sm py-2">—</p>
            ) : (
              rightBarFiles.slice(0, rightBarFilesVisible).map((a, i) => (
                <div key={`${a.url}-${i}`} className="group/file flex items-center gap-2 p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-card-dark text-sm text-gray-200 min-w-0">
            ...
            {rightBarFiles.length > rightBarFilesVisible && (
              <button type="button" disabled={loadMoreFiles} onClick={setRightBarFilesVisibleCount} className="w-full py-2 rounded-lg bg-white dark:bg-card-dark hover:bg-white/10 text-sm text-primary font-medium disabled:opacity-50 flex items-center justify-center gap-1">
        ...
        <div>
          <h5 className="text-xs font-bold text-slate-400 dark:text-gray-500 uppercase tracking-widest mb-4">{t('messages.linkSection')}</h5>
          <div className="space-y-1">
            {rightBarLinks.length === 0 ? (
              <p className="text-slate-400 dark:text-gray-500 text-sm py-2">—</p>
            ) : (
              rightBarLinks.slice(0, rightBarLinksVisible).map((item, i) => (
                <div key={`${item.url}-${item.messageId}-${i}`} className="group/link flex items-center gap-2 p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-card-dark text-sm min-w-0">
            ...
            {rightBarLinks.length > rightBarLinksVisible && (
              <button type="button" disabled={loadMoreLinks} onClick={setRightBarLinksVisibleCount} className="w-full py-2 rounded-lg bg-white dark:bg-card-dark hover:bg-white/10 text-sm text-primary font-medium disabled:opacity-50 flex items-center justify-center gap-1">
      ...
      {openMemberMenuUserId && memberMenuPosition && (() => {
        ...
        return createPortal(
          <div
            ...
            className="fixed py-1 min-w-[180px] rounded-xl bg-white dark:bg-card-dark border border-slate-200 dark:border-border-dark shadow-xl z-[9999]"
            style={{ top: memberMenuPosition.top, left: memberMenuPosition.left }}
          >
            {showSetAdmin && (
              <button type="button" onClick={() => { onSetMemberAdmin(selected.id, m.userId); closeMemberMenu() }} className="w-full px-4 py-2 text-left text-sm text-slate-900 dark:text-white hover:bg-white/10 flex items-center gap-2">
                <span className="material-symbols-outlined text-lg">admin_panel_settings</span>
                {t('messages.setAsAdmin')}
              </button>
            )}
            {onMessageUser && (
              <button type="button" onClick={() => { onMessageUser(m.userId); closeMemberMenu() }} className="w-full px-4 py-2 text-left text-sm text-slate-900 dark:text-white hover:bg-white/10 flex items-center gap-2">
                <span className="material-symbols-outlined text-lg">chat</span>
                {t('messages.messageUser')}
              </button>
            )}
            {onBlockUserInChat && (
              <button type="button" onClick={() => { onBlockUserInChat(m.userId); closeMemberMenu() }} className="w-full px-4 py-2 text-left text-sm text-slate-900 dark:text-white hover:bg-white/10 flex items-center gap-2">
                <span className="material-symbols-outlined text-lg">block</span>
                {t('messages.block')}
              </button>
            )}
            {onReport && (
              <button type="button" onClick={() => { onReport(m.userId); closeMemberMenu() }} className="w-full px-4 py-2 text-left text-sm text-slate-900 dark:text-white hover:bg-white/10 flex items-center gap-2">
