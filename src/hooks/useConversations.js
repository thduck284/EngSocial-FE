    ...
    function attachListeners(socket) {
      socket.on('conversation:read', (payload) => {
        ...
        setConversations((prev) =>
          prev.map((c) => (c.id === conversationId ? { ...c, lastMessageSeen: true } : c))
        )
            ...
            const onlyGif =
              ...
              ((Array.isArray(message.attachments) &&
                ...
                message.attachments.every(
                  (a) => (a.type || '').toLowerCase() === 'image/gif'
                )) ||
      ...
      socket.on('conversation:messageReaction', (payload) => {
        ...
        setMessages((prev) => {
          ...
          const lastReaction =
            Array.isArray(reactions) && reactions.length ? reactions[reactions.length - 1] : null
          if (isMyMessage && lastReaction?.emoji && setReactionNotification) {
            const userName = selectedNameRef?.current || t('messages.someone')
            setTimeout(
              () => setReactionNotification({ messageId, userName, emoji: lastReaction.emoji }),
              0
            )
          }
          return prev.map((m) =>
        ...
        setConversations((prev) =>
          prev.map((c) => {
            ...
            const members = Array.isArray(c.members)
              ? c.members.filter((m) => String(m.userId) !== String(userId))
              : c.members
            return {
              ...c,
              members,
              memberCount: Math.max(
                0,
                (c.memberCount ?? (c.members?.length ?? 0)) - 1
              ),
            }
          })
        )
            ...
            cPrev.map((c) =>
              c.id !== conversationId
                ...
                : {
                    ...
                    time: newLast?.createdAt
                      ? formatConversationTime(newLast.createdAt)
                      : '',
                    lastMessageFromMe: newLast?.fromMe ?? c.lastMessageFromMe,
                    lastMessageSeen: newLast?.fromMe
                      ? newLast?.read ?? c.lastMessageSeen
                      : c.lastMessageSeen,
                  }
            )
          )
    ...
    (conversationId) => {
      ...
      conversationService
        ...
        .finally(() => {
          ...
          navigate(ROUTES.MESSAGES_CONVERSATION(conversationId), {
            replace: true,
            state: {},
          })
        })
    },
    [normalizeLastMessage, navigate]
  )
