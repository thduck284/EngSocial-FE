 ...
 */
export const usesnakeGameLobby = ({
  enabled,
  token,
  capacity,
  joinCode,
  ...
  useEffect(() => {
    ...
    const lobbyRx = (event, detail) => {
      if (import.meta.env?.DEV) {
        console.log('[snakeGameLobby] ←', event, detail !== undefined ? detail : '')
      }
    }
    ...
    function attachCoreListeners(s) {
      s.on('snakeGameLobby:state', (state) => {
        lobbyRx('snakeGameLobby:state', {
      ...
      s.on('snakeGameLobby:chatMessage', (msg) => {
        if (msg && typeof msg === 'object') {
          lobbyRx('snakeGameLobby:chatMessage', { from: msg.name, len: String(msg.text || '').length })
          setChatTail((prev) => [...prev.slice(-99), msg])
        }
      })
      s.on('snakeGameLobby:matching', () => {
        lobbyRx('snakeGameLobby:matching', { socketId: s.id })
        setIsMatchingLocal(true)
        onMatchingStartedRef.current?.()
      })
      s.on('snakeGameLobby:matchingEnd', () => {
        lobbyRx('snakeGameLobby:matchingEnd', { socketId: s.id })
        setIsMatchingLocal(false)
        onMatchingEndedRef.current?.()
      })
      s.on('snakeGameLobby:started', (data) => {
        lobbyRx('snakeGameLobby:started', {
      ...
      s.on('snakeGameLobby:inviteReceived', (data) => {
        lobbyRx('snakeGameLobby:inviteReceived', { roomCode: data?.roomCode, inviterId: data?.inviterId })
        if (typeof onInviteReceivedRef.current === 'function') {
          onInviteReceivedRef.current(data)
        }
      })
      s.on('snakeGameLobby:matchFoundGlobal', (data) => {
        const myId = myUserIdRef.current != null ? String(myUserIdRef.current) : ''
        const hit = myId && data?.userIds?.map(String).includes(myId)
        lobbyRx('snakeGameLobby:matchFoundGlobal', { roomCode: data?.roomCode, forMe: hit })
        if (hit) {
          onGameStartedRef.current?.(data)
          setIsMatchingLocal(false)
        }
    ...
    socket.on('connect', () => {
      ...
      if (joinCodeNorm) {
        socket.emit('snakeGameLobby:join', { roomCode: joinCodeNorm }, done)
      } else if (cap != null && [2, 4, 6, 8].includes(cap)) {
        socket.emit('snakeGameLobby:create', { capacity: cap }, done)
      }
    })
    ...
    socket.on('connect_error', () => {
      ...
      socket.on('connect', () => {
        ...
        if (joinCodeNorm) socket.emit('snakeGameLobby:join', { roomCode: joinCodeNorm }, done)
        else if (cap != null && [2, 4, 6, 8].includes(cap)) {
          socket.emit('snakeGameLobby:create', { capacity: cap }, done)
        }
      })
  ...
  const setReady = useCallback((ready) => {
    socketRef.current?.emit('snakeGameLobby:setReady', { ready }, () => {})
  }, [])

  const sendChat = useCallback((text) => {
    socketRef.current?.emit('snakeGameLobby:chat', { text }, () => {})
  }, [])
  ...
  const findMatch = useCallback((cap) => {
    ...
    setIsMatchingLocal(true)
    onMatchingStartedRef.current?.()
    socketRef.current?.emit('snakeGameLobby:findMatch', { capacity: cap }, (res) => {
      if (!res?.ok) {
  ...
  const create = useCallback((cap) => {
    setInitError(null)
    setStartError(null)
    socketRef.current?.emit('snakeGameLobby:create', { capacity: cap }, (res) => {
      if (res?.ok && res.state) {
        setRoomState(res.state)
  ...
  const startGame = useCallback(() => {
    ...
    const attempt = () => {
      if (import.meta.env?.DEV) {
        console.log('[snakeGameLobby] → emit snakeGameLobby:start', { socketId: socketRef.current?.id })
      }
      socketRef.current?.emit('snakeGameLobby:start', {}, (res) => {
        if (import.meta.env?.DEV) {
          console.log('[snakeGameLobby] ← ack snakeGameLobby:start', res)
        }
        if (res?.ok) {
          stopStartLoop()
  ...
  const cancelStartMatching = useCallback(() => {
    stopStartLoop()
    setIsMatchingLocal(false)
    socketRef.current?.emit('snakeGameLobby:cancelStart', {}, () => {})
  }, [stopStartLoop])
  ...
  return {
    ...
    cancelStartMatching,
    leaveRoom: () => {
      socketRef.current?.emit('snakeGameLobby:leave')
      setRoomState(null)
    },
    disconnectSocket,
    inviteUrl,
