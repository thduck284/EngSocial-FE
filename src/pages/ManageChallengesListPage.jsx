...
/** @typedef {'title' | 'type' | 'skill' | 'xpReward' | 'status'} SortColumn */
...
const TYPE_ORDER = { special: 0, daily: 1, weekly: 2, monthly: 3 }

const SKILL_ORDER = { all: 0, reading: 1, listening: 2, writing: 3 }

function challengeTypeLabel(t, type) {
  const key = {
    special: 'manageChallenges.typeSpecial',
...
export function ManageChallengesListPage() {
  const { t } = useTranslation()
  const { userId } = useParams()
  const basePath = ROUTES.MANAGE_CHALLENGES(userId)
  ...
  const load = useCallback(() => {
    ...
    setLoading(true)
    challengesService
      .getChallenges({ status: 'all', limit: 500, page: 1 })
      .then((res) => {
      ...
      .catch(() => {
        setRows([])
        setError(t('manageChallenges.listLoadError'))
      })
  ...
  const filteredRows = useMemo(() => {
    ...
    if (q) {
      list = list.filter((r) => {
        const hay = [r.title, r.titleVi, r.description, r.type, r.status, r.skill].filter(Boolean).join(' ').toLowerCase()
  ...
  const emptyMessage =
    !loading && rows.length > 0 && filteredRows.length === 0
      ? t('manageChallenges.listNoMatches')
      : t('manageChallenges.listEmpty')

  return (
    <div className="p-4 md:p-6 max-w-6xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          <h1 className="text-xl md:text-2xl font-bold text-white">{t('manageChallenges.listTitle')}</h1>
          <p className="text-xs text-gray-500 mt-1">{t('manageChallenges.listSubtitle')}</p>
        </div>
        <Link
          to={newPath}
        ...
        >
          ...
          {t('manageChallenges.addChallengeBtn')}
        </Link>
      </div>
      ...
      <div className="rounded-xl border border-border-dark bg-card-dark/80 backdrop-blur-sm p-4 md:p-5 mb-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
          <div>
            <label className="block text-xs font-medium text-gray-400 mb-1.5">{t('manageChallenges.filterType')}</label>
            <select
              value={typeFilter}
            ...
            >
              ...
              <option value="special">{t('manageChallenges.typeSpecial')}</option>
              <option value="daily">{t('manageQuests.typeDaily')}</option>
              <option value="weekly">{t('manageQuests.typeWeekly')}</option>
              <option value="monthly">{t('quests.monthly')}</option>
            </select>
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-400 mb-1.5">{t('manageChallenges.filterStatus')}</label>
            <select
              value={statusFilter}
            ...
            >
              <option value="all">{t('manageWordScramble.all')}</option>
              <option value="upcoming">{t('manageChallenges.statusUpcoming')}</option>
              <option value="active">{t('manageQuests.statusActive')}</option>
              <option value="ended">{t('manageChallenges.statusEnded')}</option>
            </select>
          </div>
        </div>
          ...
          <table className="w-full text-sm text-left min-w-[720px]">
            <thead>
              <tr className="border-b border-border-dark">
                ...
                <SortableTh columnKey="skill" label={t('manageQuests.skill')} activeKey={sortKey} sortDir={sortDir} onSort={onSortColumn} t={t} />
                <SortableTh
                  columnKey="xpReward"
                ...
                pagedRows.map((row) => {
                  ...
                  return (
                    <tr key={id} className="border-b border-border-dark/80 hover:bg-white/[0.02]">
                      ...
                      <td className="px-4 py-3 text-gray-300 text-xs">{challengeTypeLabel(t, row.type)}</td>
                      <td className="px-4 py-3 text-gray-300 capitalize text-xs">{row.skill || '—'}</td>
